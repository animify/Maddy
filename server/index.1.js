...
app.prepare().then(() => {
    ...
    server.post('/hooks/v1/:id', async function(req, res, next) {
        ...
        if (req.body.action === 'created' && req.body.release) {
            const releaseMessage = `:package: New release *${req.body.release.name}* created in repository *${
                req.body.repository.name
            }* (<${req.body.repository.html_url}|@${req.body.repository.full_name}>).\n:label: Tagged *${req.body.release.tag_name}* by <${
                req.body.release.author.html_url
